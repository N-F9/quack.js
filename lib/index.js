            ...
            this.CreateEvent({
                ...
                execute(client, message) {
                    ...
                    if (message.content.startsWith(QuackJS.config.prefix)) {
                        const unformattedArgs = message.content.slice(QuackJS.config.prefix.length).split(/ +/);
                        ...
                        let args = QuackJS.config.doubleQuoteArgs ? generateDobuleQuoteArgs() : unformattedArgs;
                        args = QuackJS.config.parseArgs ? parseArgs(args) : args;
                        const vars = [_.findIndex(QuackJS.commands, { name: commandName }), _.findIndex(QuackJS.commands, (e) => e.aliases.includes(commandName))].filter((e) => e !== -1);
            ...
            this.CreateEvent({
                ...
                execute(client) {
                    ...
                    if (new Set(commandsNames).size !== commandsNames.length)
                        (0, log_1.default)('Two or more commands have the same name!', 'w');
    ...
    YAML() {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                ...
                if (Object.values(this.config.configs).length === Object.values(this.configs).length)
        ...
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                const getAllFiles = (dirPath, arrayOfFiles) => {
                    ...
                    files.forEach((file) => {
                        if (fs.statSync(path.join(process.env.PWD, dirPath, '/', file)).isDirectory()) {
